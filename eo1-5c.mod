...
var X {b in B};
var L_t;
var L_c {i in R, j in C};
var Q {i in R, j in C};
var BC {0 .. 8};

minimize Objective: (sum {i in R, j in C} (d_t - L_t + L_c[i, j] - d_c[i, j])) + w * (sum {k in 0 .. 8} BC[k]);

subject to Nonnegativity {b in B}: X[b] >= 0;

subject to AdjTumor: d_t - L_t >= 0;

subject to AdjCritical {i in R, j in C}: L_c[i, j] - d_c[i, j] >= 0;

subject to Critical {i in R, j in C}: Cr[i, j] * Q[i, j] <= L_c[i, j] * Cr[i, j];
